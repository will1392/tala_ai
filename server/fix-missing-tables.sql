-- Fix missing tables for Tala AI
-- Generated: 2025-08-05

-- 1. Create user_profiles table (from create-user-profiles-table.sql)
...
-- Create policies for conversation_contexts
CREATE POLICY "Users can access their own contexts" ON conversation_contexts
    FOR ALL USING (user_id = current_setting('request.jwt.claims', true)::json->>'sub');

CREATE POLICY "Service role can access all contexts" ON conversation_contexts
...
-- 3. Fix the migrations table status column
ALTER TABLE migrations ADD COLUMN IF NOT EXISTS status VARCHAR(50) DEFAULT 'completed';

-- 4. Fix RLS policies for users table (to prevent infinite recursion)
