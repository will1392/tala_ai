...
export class EnhancementStage extends Stage {
  ...
  async shouldProcess(input, context) {
    // Check if response should be final (no enhancement needed)
    if (ResponseContract.shouldBeFinal(input)) {
      this.log('info', 'Response is final, skipping enhancement');
      return false;
    }
    
    // Never enhance if response is from specialized agent and flag is set
    if (this.preserveSpecialized && ResponseContract.isSpecializedResponse(input)) {
