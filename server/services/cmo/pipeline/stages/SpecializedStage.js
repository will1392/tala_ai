...
export class SpecializedStage extends Stage {
  ...
  async shouldProcess(input, context) {
    ...
    // Debug context
    this.log('debug', 'Context keys:', Object.keys(context));
    this.log('debug', 'Context.detected:', context.detected);
    
    // Skip if no channel detected
    if (!context.detected?.channel) {
      this.log('debug', 'No channel detected, skipping');
      this.log('debug', 'Full context:', JSON.stringify(context, null, 2));
      return false;
    }
  ...
  async process(input, context) {
    ...
    try {
      ...
      this.log('info', 'Agent execution complete', {
        ...
        agentResultType: agentResult?.type,
        contentPreview: agentResult?.content?.text?.substring(0, 100) || 'no text'
      });
      
      // Convert agent result to CMOResponse
      const response = this.convertAgentResponse(agentResult, agentChannel);
      
      this.log('info', 'Converted to CMOResponse', {
